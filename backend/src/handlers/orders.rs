...
pub fn orders_router() -> Router<AppState> {
    Router::new().route("/orders", get(list_orders).post(create_order)).route("/orders/catalog", get(list_catalog)).route("/orders/:order_id", get(get_order)).route("/orders/:order_id/status", put(update_status))
}

async fn list_orders(State(state): State<AppState>, auth: AuthUser<OrderReadAccess>, Query(query): Query<OrdersQuery>) -> Result<impl IntoResponse, OrderError> { Ok(Json(state.order_service.list_orders(auth.user.tenant_id, query.status).await?)) }
...
async fn get_order(State(state): State<AppState>, auth: AuthUser<OrderReadAccess>, Path(order_id): Path<Uuid>) -> Result<impl IntoResponse, OrderError> { Ok(Json(state.order_service.get_order(auth.user.tenant_id, order_id).await?)) }
async fn create_order(State(state): State<AppState>, auth: AuthUser<OrderWriteAccess>, Json(payload): Json<CreateOrderPayload>) -> Result<impl IntoResponse, OrderError> {
    ...
    Ok((StatusCode::CREATED, Json(state.order_service.create_order(auth.user.tenant_id, auth.user.id, order).await?)))
}
async fn update_status(State(state): State<AppState>, auth: AuthUser<OrderWriteAccess>, Path(order_id): Path<Uuid>, Json(payload): Json<UpdateStatusPayload>) -> Result<impl IntoResponse, OrderError> { Ok(Json(state.order_service.update_status(auth.user.tenant_id, order_id, payload.status).await?)) }
