...
use uuid::Uuid;
use crate::{application::orders::OrderError, domain::orders::{NewOrder, NewOrderItem, OrderStatus, PaymentMethod, ServiceType}, presentation::auth::{extractor::AuthUser, policy::{OrderReadAccess, OrderStatusUpdateAccess, OrderWriteAccess, SalesReadAccess}}, AppState};
...
async fn sales_summary(State(state): State<AppState>, auth: AuthUser<SalesReadAccess>) -> Result<impl IntoResponse, OrderError> { Ok(Json(state.order_service.sales_summary(auth.user.tenant_id, auth.branch).await?)) }
...
async fn update_status(State(state): State<AppState>, auth: AuthUser<OrderStatusUpdateAccess>, Path(order_id): Path<Uuid>, Json(payload): Json<UpdateStatusPayload>) -> Result<impl IntoResponse, OrderError> { Ok(Json(state.order_service.update_status(auth.user.tenant_id, auth.branch, order_id, payload.status).await?)) }
